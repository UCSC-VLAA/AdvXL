...
Modified by Zeyu Wang, Copyright 2024
"""
import argparse
import os
import csv
...
#  attacker options
parser.add_argument('--eval-attack-iters', help='Adversarial attack iteration during evaluation', type=int, nargs='+', default=0)
parser.add_argument('--eval-attack-epsilons', help='Adversarial attack maximal perturbation during evaluation', type=float, nargs='+', default=1.0)
parser.add_argument('--eval-attack-step-sizes', help='Adversarial attack step size during evaluation', type=float, nargs='+', default=1.0)
parser.add_argument('--eval-prob-start-from-cleans', type=float, default=0, nargs='+')
...
def validate(args):
    ...
    ################
    if args.eval_attack_iters != 0:
        adv_model = AdvModel(model)
        all_results = OrderedDict()
        for eval_attack_iter, eval_attack_epsilon, \
            eval_attack_step_size, eval_prob_start_from_clean in \
                zip(args.eval_attack_iters, args.eval_attack_epsilons,
                    args.eval_attack_step_sizes, args.eval_prob_start_from_cleans):
            if eval_attack_iter != 0:
                eval_attacker = PGDAttacker(eval_attack_iter, eval_attack_epsilon, eval_attack_step_size,
                                            dev_env=dev_env,
                                            prob_start_from_clean=eval_prob_start_from_clean,
                                            mean=data_config['mean'], std=data_config['std'])
            else:
                eval_attacker = NoOpAttacker()
            adv_model.set_attacker(eval_attacker)
            results = run_eval(args, adv_model, loader, dev_env,
                               valid_labels, real_labels, criterion,
                               param_count, data_config, eval_pp_cfg, adversarial=True)
            for key, val in results.items():
                if eval_attack_iter != 0:
                    all_results[
                        f'pgd_{eval_attack_iter}_{eval_attack_epsilon}_{eval_attack_step_size}_{eval_prob_start_from_clean}_' + key] = val
                else:
                    all_results[key] = val
        results = all_results
    else:
        results = run_eval(args, model, loader, dev_env,
                           valid_labels, real_labels, criterion,
                           param_count, data_config, eval_pp_cfg, adversarial=False)
    #################

    return results


def run_eval(args, model, loader, dev_env,
             valid_labels, real_labels, criterion,
             param_count, data_config, eval_pp_cfg, adversarial=False):
    ...
    with torch.no_grad():
        ...
        for step_idx, (sample, target) in enumerate(loader):
            ...
            with dev_env.autocast():
                if adversarial:
                    output = model(sample, target, criterion)
                else:
                    output = model(sample)
