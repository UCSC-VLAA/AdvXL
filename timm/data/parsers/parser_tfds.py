...
SHUFFLE_SIZE = int(os.environ.get('TFDS_SHUFFLE_SIZE', 256))  # samples to shuffle in DS queue
...
def get_class_labels(info):
    ...
    if not hasattr(class_label, 'names'):
        return {}
    else:
...
class ParserTfds(Parser):
    ...
    def _lazy_init(self):
        ...
        if not self.worker_init:
            ...
            self.num_workers = 1
            self.global_num_workers = self.dist_num_replicas * self.num_workers
            self.global_worker_id = self.dist_rank
        ...
        read_config = tfds.ReadConfig(
            shuffle_seed=self.common_seed + self.epoch_count.value,  # shard shuffling seed
        ...
        if self.is_training:
            ...
            ds = ds.shuffle(
                ...
                min(self.num_samples, self.shuffle_size),
    ...
    def __iter__(self):
        ...
        target_example_count = self._num_samples_per_worker()
