...
def create_loader_v2(
    ...
    if dev_env.distributed and not isinstance(dataset, torch.utils.data.IterableDataset):
        if is_training:
            ...
            if pp_cfg.aug.num_aug_repeats:
                # sampler = RepeatAugSampler(dataset, num_repeats=num_aug_repeats)
                sampler = RepeatAugSampler(dataset, num_replicas=dev_env.world_size, rank=dev_env.global_rank, num_repeats=pp_cfg.aug.num_aug_repeats)
            else:
                sampler = torch.utils.data.distributed.DistributedSampler(dataset, num_replicas=dev_env.world_size, rank=dev_env.global_rank)
        else:
