...
def unwrap_model(model, recursive=True, extra_sub_module_attrs=()):
    for attr in _SUB_MODULE_ATTR + extra_sub_module_attrs:
        sub_module = getattr(model, attr, None)
        if sub_module is not None:
            return unwrap_model(sub_module, extra_sub_module_attrs=extra_sub_module_attrs) if recursive else sub_module
    return model
...
def state_dict_tp_fp32(state_dict):
    for key, val in state_dict.items():
        state_dict[key] = val.to(torch.float32)
    return state_dict
...
def auto_resume_helper(output_dir):
    ...
    _logger.info(f"All checkpoints founded in {output_dir}: {checkpoints}")
    if len(checkpoints) > 0:
        latest_checkpoint = max([os.path.join(output_dir, d) for d in checkpoints], key=os.path.getmtime)
        _logger.info(f"The latest checkpoint founded: {latest_checkpoint}")
