...
try:
    ...
    _HAS_XLA_CHECKPOINT = True
except:
    _HAS_XLA_CHECKPOINT = False
...
def clean_state_dict(state_dict):
    ...
    for k, v in state_dict.items():
        ...
        # for adv model
        name = name[11:] if name.startswith('base_model.') else name
...
def checkpoint_seq(
        ...
        use_xla_checkpoint=False, # use checkpoint from pytorch xla
):
    ...
    for start in range(0, num_checkpointed, every):
        ...
        if use_xla_checkpoint:
            assert _HAS_XLA_CHECKPOINT, 'Please install PyTorch-XLA first. Make sure this is a TPU machine'
            x = xla_checkpoint(run_function(start, end, functions), x, preserve_rng_state=preserve_rng_state)
        else:
